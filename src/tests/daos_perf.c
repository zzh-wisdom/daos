...
unsigned int		 ts_akey_p_dkey	= 128;	/* # akeys per dkey */
unsigned int		 ts_recx_p_akey	= 1024;	/* # recxs per akey */
...
struct pf_param {
	...
	union {
		...
		/* private parameter for fetch and update */
		struct {
...
stride_buf_op(int opc, char *buf, unsigned offset, int size)
{
	...
	if (opc == STRIDE_BUF_WRITE) {
		stride_buf.sb_mark++;
		if (stride_buf.sb_mark > 'Z')
			stride_buf.sb_mark = 'A';
	}

	for (i = (offset & ~((1U << 12) - 1));
	     i < stride_buf.sb_size; i += (1 << 12)) {
		for (j = 0; j < ARRAY_SIZE(stride_marks); j++) {
			...
			if (pos >= offset + size) {
				 ...
				 */
				if (ts_single && opc == STRIDE_BUF_WRITE)
					mark = 0;
				else
					return true;
			}
...
static void
stride_buf_write(unsigned offset, int size)
{
	stride_buf_op(STRIDE_BUF_WRITE, NULL, offset, size);
}

static void
stride_buf_read(char *buf, unsigned offset, int size)
{
	stride_buf_op(STRIDE_BUF_READ, buf, offset, size);
}

static bool
stride_buf_equal(char *buf, unsigned offset, int size)
{
	return stride_buf_op(STRIDE_BUF_EQUAL, buf, offset, size);
}
		     ...
		     int idx, struct pf_param *param)
{
	...
	if (op_type == TS_DO_UPDATE) {
		...
		stride_buf_read(cred->tc_vbuf, param->pa_rw.offset,
				param->pa_rw.size);
	} else {
	...
	if (param->pa_rw.verify) {
		rc = stride_buf_equal(cred->tc_vbuf, param->pa_rw.offset,
				      param->pa_rw.size);
...
objects_update(struct pf_param *param)
{
	...
	stride_buf_write(param->pa_rw.offset, param->pa_rw.size);
	++epoch;
...
run_commands(char *cmds)
{
	...
	while (1) {
		...
		if (ts) {
			...
			if (rc) {
				D_PRINT("Invalid test parameters\n");
				return rc;
			}
...
-A [R]\n\
	...
	optional paramter 'R' indicates random writes\n\
\n\
-s number\n\
...
main(int argc, char **argv)
{
	...
	while ((rc = getopt_long(argc, argv,
				 "P:N:T:C:c:o:d:a:n:R:Dg:G:s:S:zf:hBwxpA::",
				 ts_ops, NULL)) != -1) {
		char	*endp;
